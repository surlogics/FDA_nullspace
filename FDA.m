...
Z=0;
    ...
    Xtilt=trans*X;
    %{
    LocalMut = cell(1,NumberOfClasses);
    CovValt = cell(1,NumberOfClasses);
    sizeC=zeros(1,NumberOfClasses);
    % Compute local Mu, cov matrix, and number of observation of
    % for each class class
    for clsLbl=unique(Y')
        Xct=Xtilt(:,Y==clsLbl);
        LocalMut(i) = {mean(Xct,2)};
        CovValt{i} = bsxfun(@minus,Xct,LocalMut{i})*bsxfun(@minus,Xct,LocalMut{i})';
        sizeC(i)=size(Xct,2);
        i=i+1;
    end
    Global_Mut = zeros(d,1);
    
    for i = 1:NumberOfClasses 
        Global_Mut = Global_Mu+LocalMut{i};
    end
    Global_Mut=Global_Mut./NumberOfClasses;
    %SB: Betweeness class scatter matrix
    %SW: Scatter Class Matrix
    ...
    for i = 1:NumberOfClasses
        SB = SB + sizeC(i).*(LocalMut{i}-Global_Mu)*(LocalMut{i}-Global_Mu)';
        SW = SW+CovValt{i};
    end
